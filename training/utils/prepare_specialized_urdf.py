#!/usr/bin/env python3
"""
This code was taken and adapted from 'stretch_dex_teleop' (Hello Robot Inc.).

GOAL: Generate an 8-DOF "Omni-Mobile" URDF for the Stretch 3. 
By nesting the virtual 'joint_mobile_base_translation' joint as a child of the 
'joint_mobile_base_rotation' joint, we resolve the non-holonomic constraint 
mathematically. This ensures the robot's forward translation axis always 
aligns with its current base rotation, allowing the IK solver to command 
fluid whole-body mobile manipulation for the Universal Kitchen Assistant.
"""
import pathlib
...
# This adds the directory containing dex_teleop_parameters.py to your search path
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DEX_TELEOP_DIR = os.path.join(SCRIPT_DIR, '..', '..', 'dependencies', 'stretch_dex_teleop')
sys.path.insert(0, DEX_TELEOP_DIR)
...
non_fixed_joints = [
    'joint_mobile_base_translation', # Added for 8-DOF
    'joint_mobile_base_rotation',    # Added for 8-DOF
    'joint_lift',
    'joint_arm_l0',
    'joint_arm_l1',
    'joint_arm_l2',
...
if use_original_limits:
    ...
    ik_joint_limits = {
        'joint_mobile_base_translation' : (None, None), # Added
        'joint_mobile_base_rotation' : (None, None),    # Added
...
###############################################
# CREATE 8-DOF NESTED OMNI-MOBILE BASE
...
robot_omni.add_joint(ud.Joint(
    ...
    parent='virtual_base',
    child='base_rotation_link',
    joint_type='revolute',
    axis=[0, 0, 1],
    origin=ud.Pose(xyz=[0, 0, 0], rpy=[0, 0, 0]),
...
robot_omni.add_joint(ud.Joint(
    ...
    parent='base_rotation_link',
    child='base_link',
    joint_type='prismatic',
    axis=[1, 0, 0],
    origin=ud.Pose(xyz=[0, 0, 0], rpy=[0, 0, 0]),
    limit=ud.JointLimit(effort=10, velocity=1, lower=-1.0, upper=1.0)
))
...
print()
save_urdf_file(robot_omni, 'stretch_omni_mobile_ik.urdf')

# Create version with fixed wrist
print('Prepare 8-DOF URDF with a fixed wrist.')
robot_fixed_wrist = deepcopy(robot_omni)
non_fixed_base_joints = [
...
for j in robot_fixed_wrist.joint_map.keys():
    if j not in non_fixed_base_joints:
        robot_fixed_wrist.joint_map[j].type = 'fixed'
            
save_urdf_file(robot_fixed_wrist, 'stretch_omni_mobile_ik_with_fixed_wrist.urdf')