...
Pipeline:
  ...
  3. Forward {rgb, state} to sheep (4406)
  4. Recv (8,) action from sheep (4405)
  5. Re-anchor T_t0_robot every CHUNK_SIZE steps (fixed clock, matches policy queue)
  6. PD2.1 + IK → 9D joints → vtam_robot_node (4402)
...
from training.scripts.replay_demo import OnlineIK
...
CHUNK_PORT  = PORTS['chunk']     # 4405 — recv action from sheep
...
def main():
    ...
    print(f"  obs->{FWD_PORT}(sheep)  action<-{CHUNK_PORT}(sheep)")
    ...
    action_in_sock = ctx.socket(zmq.SUB)
    action_in_sock.setsockopt(zmq.SUBSCRIBE, b"")
    action_in_sock.setsockopt(zmq.CONFLATE, 1)
    action_in_sock.setsockopt(zmq.RCVTIMEO, 5000)
    action_in_sock.bind(f"tcp://*:{CHUNK_PORT}")

    action_out_sock = ctx.socket(zmq.PUB)
    action_out_sock.connect(f"tcp://localhost:{ACTION_PORT}")
    ...
    try:
        while True:
            ...
            # ── 3. Re-anchor every CHUNK_SIZE steps ───────────────────────
            if step % CHUNK_SIZE == 0:
                ...
                print(f"  Re-anchored at step={step}")

            # ── 4. Forward obs to sheep ───────────────────────────────────
            try:
                fwd_sock.send(
            ...
            # ── 5. Recv action from sheep ─────────────────────────────────
            try:
                action_8d = pickle.loads(action_in_sock.recv(flags=zmq.NOBLOCK))
            except zmq.Again:
                step += 1
            ...
            # ── 6. PD2.1: T_target = T_t0_robot @ T_chunk_rel ────────────
            pos_rel   = action_8d[:3]
            quat_rel  = action_8d[3:7]
            grip_norm = float(action_8d[7])
            ...
            # ── 8. Send to vtam_robot_node ────────────────────────────────
            if not args.dry_run:
                action_out_sock.send(
        ...
        for s in [obs_sock, state_sock, fwd_sock, action_in_sock, action_out_sock]:
            s.close()
        ctx.term()
